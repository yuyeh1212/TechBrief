...
- [x] 換頁統一滾回頂部（ScrollToTop 全域元件，監聽 pathname 變化）
- [x] 友站抽屜改為膠囊形狀（縮小佔位、保持右側固定位置）
- [x] 文章頁預估閱讀時間（字數 ÷ 400 字/分鐘，顯示於 meta 列）
- [x] 個股簡評歷史查詢記錄（localStorage 最近 5 筆，可點擊重查、一鍵清除）
- [x] 右下角回頂按鈕（滾動超過 400px 才出現，平滑回頂）
